...
/** Ablage im Arbeitsspeicher — genau die zwei Methoden der Speicherhülle. */
...
// Die Ablage wird hineingereicht (ADR-003) — die Sammlung greift auf kein
// Global zu. Der Test hält sie fest, um zwischendurch in den Rohspeicher zu
// sehen.
...
describe("Automatisches Aufräumen ruhender Sammlungen", () => {
  ...
  const sammlung = (alterTage: number, over: Partial<Einsatzsammlung> = {}): Einsatzsammlung => ({
    id: `s${alterTage}`,
    name: "Übung",
  ...
  it("räumt beim Laden automatisch auf und schreibt den Stand zurück", () => {
    ...
    const roh = einsaetzeAusJson(speicher.getItem("eeb.einsaetze.v1"));
    einsaetzeSpeichern(
    ...
    expect(einsaetzeAusJson(speicher.getItem("eeb.einsaetze.v1"))).toHaveLength(1);
  });

  it("eine Änderung setzt die Frist zurück", () => {
    const s = einsatzAnlegen("Aktiv", EinsatzArt.EINSATZ);
    einsaetzeSpeichern(
      einsaetzeAusJson(speicher.getItem("eeb.einsaetze.v1")).map((x) => ({
        ...x,
