...
function neueId(): string {
  return crypto?.randomUUID?.() ?? `e${Date.now()}-${Math.random().toString(36).slice(2)}`;
}
...
export function einsatzWiederherstellen(id: string): void {
  einsaetzeSpeichern(
    alleEinsaetzeLaden().map((s) => {
      if (s.id !== id) return s;
      // Das Feld verschwindet, statt auf undefined gesetzt zu werden — die
      // Serialisierung soll es gar nicht erst mitschreiben.
      const { geloeschtAm, ...rest } = s;
      void geloeschtAm;
      return rest;
    }),
  );
